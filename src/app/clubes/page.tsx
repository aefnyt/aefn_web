...
interface ClubActividad {
  fecha: string;
  titulo: string;
  descripcion: string;
}
interface ClubDirectiva {
  cargo: string;
  nombre: string;
  email: string;
}
interface Club {
  ...
  directiva: ClubDirectiva[];
  actividades: ClubActividad[];
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <PageHeader
        ...
        title={<>Clubes <span className="text-amber-400">estudiantiles</span></>}
        description="Espacios de aprendizaje colaborativo donde los estudiantes exploran sus intereses científicos más allá del aula."
      />
          ...
          {clubes.map((club, i) => (
            ...
            >
              <div>
                ...
                {club.contacto_email && (
                  <a
                    href={`mailto:${club.contacto_email}`}
                ...
                {club.directiva.length > 0 && (
                  <div>
                    ...
                    <ul className="space-y-1.5">
                      {club.directiva.map((d, j) => (
                        <li key={j} className="flex items-baseline gap-2 text-sm">
                          <span className="text-neutral-900 font-normal">{d.nombre}</span>
                          <span className="text-neutral-400 font-light">— {d.cargo}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
                ...
                {club.actividades.length > 0 && (
                  <div>
                    ...
                    <ul className="space-y-2">
                      {club.actividades.map((a, j) => (
                        <li key={j} className="text-sm">
                          <span className="text-amber-500 font-light">{a.fecha}</span>
                          <span className="text-neutral-700 font-light ml-2">{a.titulo}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
