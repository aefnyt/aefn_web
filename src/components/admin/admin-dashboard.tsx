            ...
            {allowedModules.map((module) => {
              ...
              return (
                <Link key={module.key} href={`/admin/${module.key}`}>
                  <Card className="group h-full cursor-pointer relative overflow-hidden aefn-card aefn-glow-hover">
                    <CardHeader>
                      <div className="flex items-center justify-between mb-2">
                        <div className="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-neutral-950 text-amber-400 group-hover:shadow-[0_0_22px_rgba(255,215,0,0.55)] transition-shadow">
                          <Icon className="w-5 h-5" />
                        </div>
                        <ArrowRight className="w-4 h-4 text-neutral-300 group-hover:text-amber-600 group-hover:translate-x-1 transition-all" />
                      </div>
                      <CardTitle className="text-lg group-hover:text-amber-700 transition-colors">{module.label}</CardTitle>
                      <CardDescription>{module.description}</CardDescription>
                    </CardHeader>
                    <CardContent>
                      <p className="text-xs text-neutral-400 aefn-mono flex items-center gap-1.5">
